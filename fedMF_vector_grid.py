...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('grid_search_federated_vector.log', mode='a'),
    ]
)
...
CSV_PATH   = 'grid_results_federated_vector.csv'
CSV_FIELDS = ['hidden_dim', 'lr', 'reg', 'n_iter', 'aggregation_int', 'hr_at_k', 'total_time_s']

# ── Treino local ───────────────────────────────────────────────────────────────
def user_update(user_vector, v, lr, reg_u, reg_v, n_iter):
    v_local = v.copy()
...
def aggregate_fedavg(items_matrix_global, items_matrix_local, interact_count,
                     updated_items, item_users_updated, item_users_all, hidden_dim):
...
def run(hidden_dim, lr, reg, n_iter, aggregation_int, train_end, validation_end):
    ...
    for obs_count, (uid, item_id) in enumerate(stream_data[:validation_end], start=1):
        ...
        users_matrix[user_index], items_matrix_local[uid][item_id] = user_update(
            users_matrix[user_index], items_matrix_local[uid][item_id],
            lr, reg, reg, n_iter
        )
...
def main():
    ...
    logger.info(f'aggregation_int={aggregation_int}')

    logger.info(f'Grid search federado | {total} combinações | Dataset={n} | Train={train_end} | Val={validation_end - train_end}')
